...
@util_plugin.command
@lb.option(
    "image", "The image in question", str, 
)
@lb.option(
    "tag", "The ???", str 
)
...
@util_plugin.command
@lb.option(
    "emoji", "The emoji to remove", hk.Emoji, 
)
...
@util_plugin.command
@lb.option(
    "thumbnail", "The url of the thumbnail, if any", str, required=False
)
@lb.option(
    "link", "The redirect url of the title", str, required=False
)
@lb.option(
    "footer", "Add a requested by: footer?", bool, required=False
)
@lb.option(
    "image", "The url of the image, if any", str, required=False
)
@lb.option(
    "title", "The title of the embed", str ,required=False 
)
@lb.option(
    "content", "The content of the embed", str, 
)
@lb.command("embed", "Make an embed message", pass_options=True)
@lb.implements(lb.SlashCommand)
async def make_embed(
    ctx: lb.Context, content: str, title: str=None, image: str=None, footer: bool=False, link: str=None, thumbnail: str=None
) -> None:
    embed=hk.Embed(
            description=content, title=title, url=link
            ).set_image(image).set_thumbnail(thumbnail)
...
@util_plugin.command
@lb.option(
    "image", "The url of an image, if to embed", str, required=False
)
@lb.option(
    "role2", "If one wasn't enough", hk.Role, required=False
)
@lb.option(
    "footer", "Add a from footer?", bool, required=False
)
@lb.option(
    "role", "The role to ping with the announcement, if any", hk.Role, required=False
)
@lb.option(
    "content", "The content of the announcement", str ,
)
@lb.option(
    "channel", "The channel to send the message in", hk.TextableGuildChannel, 
)
@lb.command("announce", "Make an announcement", pass_options=True)
@lb.implements(lb.SlashCommand)
async def make_embed(
    ctx: lb.Context, channel: hk.TextableGuildChannel, content: str, role: hk.Role=None, footer: bool=False, role2: hk.Role=hk.undefined.UNDEFINED, image: str=None
) -> None:
    roles = []
    if role:
...
@util_plugin.command
@lb.option(
    "text", "The text to repeat", str, modifier=lb.OptionModifier.CONSUME_REST
)
@lb.command("echo", "Repeat")
@lb.implements(lb.PrefixCommand)
async def make_embed(ctx: lb.Context) -> None:
    
    await ctx.respond(ctx.options.text, user_mentions=True, role_mentions=True, mentions_everyone=True)
...
@lb.option(
    "code", "The plugin content", str, modifier=lb.OptionModifier.CONSUME_REST
)
