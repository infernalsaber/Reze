...
al_listener = lb.Plugin("Weeb", "Search functions for anime, manga and characters (with an easter egg)")
...
async def al_link_finder(event: hk.GuildMessageCreateEvent) -> None:
    ...
    if len(listOfSeries) != 0:
        ...
        await al_listener.bot.rest.edit_message(event.channel_id, event.message, flags=hk.MessageFlag.SUPPRESS_EMBEDS)
...
'''
            ...
            if response.status_code != 200:
                print(response.json())
                await event.message.respond(f"Nvm 😵, error `code: {response.status_code}`")
                return
            ...
            await event.message.respond(content="Here are it's details", 
                embed=hk.Embed(
                    description="\n\n", 
                    ...
                    .add_field("Chapters" if response['type'] == "MANGA" else "ANIME", no_of_items, inline=True)
                    .add_field("Summary", f"{response['description'][0:250].replace('<br>', '') if len(response['description']) > 250 else response['description'].replace('<br>', '')}...")
                    .set_thumbnail(response['coverImage']['large'])
                    .set_image(response['bannerImage'])
                    .set_author(url=response['siteUrl'],name=title)
                    .set_footer(text="Source: AniList", icon="https://upload.wikimedia.org/wikipedia/commons/thumb/6/61/AniList_logo.svg/1024px-AniList_logo.svg.png",)
            )
...
'''
    ...
    if response.status_code != 200:
        print(response.json())
        await ctx.respond(f"Failed to fetch data 😵, error `code: {response.status_code}`")
        return
    response = response.json()['data']['Media']
    ...
    if response['description']:
        response['description'] = response['description'].replace("<br>", "").replace("<i>", "").replace("<b>", "").replace("</b>", "").replace("</i>", "")
...
# print([chapter["id"] for chapter in r.json()["data"]])
    ...
    preview = await ctx.respond(
        embed=hk.Embed(
            ...
            .add_field("Genres", ",".join(response['genres']))
            .add_field("Status", response['status'], inline=True)
            .add_field("Chapters" if response['type'] == "MANGA" else "Episodes", no_of_items, inline=True)
            .add_field("Summary", response['description'])
            .set_thumbnail(response['coverImage']['large'])
            .set_image(response['bannerImage'])
            .set_author(url=response['siteUrl'],name=title)
    ...
    # for item in sorted(os.listdir(f"./manga/{data['first']['id']}")):
        ...
        pages.append(
            ...
            .set_footer(
                "Fetched via: MangaDex", icon="https://avatars.githubusercontent.com/u/100574686?s=280&v=4"
            )
        )
    pages.append(
        hk.Embed(
            title=title, url=f"https://cubari.moe/read/mangadex/{manga_id}/2/1"
        )
        .set_image(response['coverImage']['large'])
        .set_author(name="Click here to continue reading", url=f"https://cubari.moe/read/mangadex/{manga_id}/2/1")
    )
    buttons = [
        nav.PrevButton(
            style=hk.ButtonStyle.SECONDARY,
...
'''
    ...
    response = await ctx.bot.d.aio_session.post('https://graphql.anilist.co', json={'query': query, 'variables': variables})
    if not response.ok:
...
async def topanime(ctx: lb.PrefixContext, filter: str = None):
    ...
    async with ctx.bot.d.aio_session.get(
        ...
        if res.ok:
            ...
            embed = hk.Embed(
                color=0x2e51a2
            ).set_author(name="Top Anime").set_footer("Fetched via MyAnimeList.net", icon="https://image.myanimelist.net/ui/OK6W_koKDTOqqqLDbIoPAiC8a86sHufn_jOI-JGtoCQ")
...
'''
    ...
    if response['description']:
        response['description'] = response['description'].replace("<br>", "").replace("~!", "||").replace("!~", "||")
