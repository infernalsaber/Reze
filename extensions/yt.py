...
class YTVideo:
        ...
        def __init__(self, searchResults: dict, i: int) -> None:
            ...
            self.vThumb: str = searchResults["items"][i]["snippet"]["thumbnails"]["high"]["url"] #make it medium later
        ...
        def set_duration(self, req) -> str:
                ...
                if self.vDuration.startswith("0:"):
                    self.vDuration = str(isodate.parse_duration(ytapi2["items"][0]["contentDetails"]["duration"]))[2:]
...
@lb.command("youtubesearch", "Search YouTube and get videos", aliases=["yts"], pass_options=True)
...
async def userinfo(ctx: lb.Context, query: str) -> None:
    ...
    choice = await ctx.edit_last_response(
        embed=hk.Embed(
            ...
            color=hk.Color.of(get_dominant_colour(Image.open(requests.get(lstVids[vidIndex].vThumb, stream=True).raw))),
    ...
    if hasattr(view2, "answer"):  # Check if there is an answer
        await ctx.edit_last_response(f"Video link: {lstVids[vidIndex].get_link()}", embeds=[], flags=hk.MessageFlag.SUPPRESS_EMBEDS, components=[])
        if not view2.answer:
            return
    else:
        await ctx.edit_last_response("Process timed out.", embeds=[], views=[])
...
@yt_plugin.command
@lb.option(
    "video", "The video url to download", str
)
